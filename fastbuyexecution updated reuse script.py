...
async def executebuy_trade():
    ...
    order_data = {
        'symbol': symbol, 'side': 'BUY', 'type': 'MARKET', 'quantity': quantity, 'timestamp': get_server_time()
    }
    order_res = await place_order(order_data)
    logger.info(f"BUY Market order placed: {order_res}")

    # SL and TP orders
    sl_data = {'symbol': symbol, 'side': 'SELL', 'type': 'STOP_MARKET', 'stopPrice': sl_price, 'closePosition': True, 'timestamp': get_server_time()}
    tp_data = {'symbol': symbol, 'side': 'SELL', 'type': 'TAKE_PROFIT_MARKET', 'stopPrice': tp_price, 'closePosition': True, 'timestamp': get_server_time()}
